<script lang="ts">
    ...
    const submitEditedRecipe = async () => {
        ...
        form.append("recipe_id", data.recipe?.id.toString() as string);
        ...
        const response = await window.fetch(endpoint("/recipes/edit"), {
            method: "POST",
            body: form,
            headers: {
        ...
        goto(`/recipes/${data.recipe?.id}`);
    };

    if (data.authorized) {
        title = data.recipe.recipe.title;
        description = data.recipe.recipe.description;
        ingredients = data.recipe.recipe.ingredients;
        steps = data.recipe.recipe.steps.map((val) => {
            return val.step_details;
        });
    }
</script>
...
<section class="flex w-full h-fit justify-center items-center">
    {#if data.authorized}
        <form
            on:submit={submitEditedRecipe}
            class="shadow-one flex flex-col gap-2 w-[580px] mt-20 p-4 bg-white"
        >
            <h1 class="text-3xl font-bold">Editing {data.recipe.recipe.title}</h1>
            <article>
                ...
                <div class="mt-1">
                    {#each ingredients as ingredient, i}
                        <div class="w-full bg-gray-100 mb-2 flex flex-row">
                            ...
                            <div
                                class="w-1/2 flex items-center justify-end p-1"
                            >
                                <button
                                    on:click={() => {
